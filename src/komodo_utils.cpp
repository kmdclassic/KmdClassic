...
uint16_t komodo_userpass(char *userpass,const char *symbol)
{
    ...
    {
#ifdef __APPLE__
        sprintf(confname,"KmdClassic.conf");
#else
        sprintf(confname,"kmdclassic.conf");
#endif
    }
...
#ifdef __APPLE__
    std::string filename = "KmdClassic.conf";
#else
    std::string filename = "kmdclassic.conf";
#endif
